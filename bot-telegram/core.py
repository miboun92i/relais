...
Renvoie exactement [RELAIS_HUMAIN] UNIQUEMENT si le client dit qu il a paye, envoie une preuve, ou est pret a payer tout de suite.
Sinon tu geres seule.
Reponds seulement avec le texte a envoyer.
'''

HANDOFF_FALLBACKS = [
    'ok je check ca et je te reviens',
    'recu, je m en occupe',
    'ok laisse-moi 2 min je te confirme',
]
    ...
    async def draft(self, chat_id, *, manual_on_handoff=True):
        ...
        async with self.capacity:
            ...
            if needs_human_output(text):
                if manual_on_handoff and '[relais_humain]' in text.lower().replace(' ', ''):
                    self.set_mode(chat_id, 'manual')
                    raise HumanHandoffRequired('Le client est pret a payer ou dit avoir paye. Reprenez pour encaisser.')
                text = text.replace('[RELAIS_HUMAIN]', '').replace('[relais_humain]', '').strip() or 'dis-moi ce que tu veux voir'
