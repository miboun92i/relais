...
const initialSettings = {enabled:true,tone:'Réponds en français, avec un ton chaleureux et naturel. Tutoie le client et reste concise. Pour toute demande qui nécessite une décision personnelle, passe-moi la main.',catalog:'',faq:'',daily_limit:100};
const demoChats = [
...
let selected = 1, filter='all', view='inbox', credentials=null, threadMessages=[], lastMessages='', refreshing=false, toastTimer, connectionEpoch=0;
...
function setView(next){view=next;$('inbox-view').hidden=next!=='inbox';$('settings-view').hidden=next!=='settings';$('page-title').textContent=next==='inbox'?'Conversations':'Mon assistant';document.querySelectorAll('.nav').forEach(b=>b.classList.toggle('active',b.dataset.view===next));if(next==='settings')loadSettings();}
